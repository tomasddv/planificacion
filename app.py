...
DEFAULT_OBJECTIVES_ROWS = [
    {"seccion": "", "item": "TOTAL CVZA", "OBJ VTAS": 4828.0},
    {"seccion": "", "item": "TOTAL UNG", "OBJ VTAS": 3459.0},
    {"seccion": "", "item": "CZA", "OBJ VTAS": 4828.0},
    {"seccion": "", "item": "UNG", "OBJ VTAS": 3459.0},
    {"seccion": "", "item": "CVZA HE", "OBJ VTAS": 1296.0},
    {"seccion": "", "item": "CVZA CORE + VALUE", "OBJ VTAS": 3532.0},
    {"seccion": "", "item": "MIX PREMIUM", "OBJ VTAS": 26.8},
...
def objective_matrix_month(source: pd.DataFrame) -> int | None:
    ...
    for _, row in source.head(12).iterrows():
        label = clean_name(row.iloc[0] if len(row) else "")
        if label != "mes":
            continue
        for value in row.iloc[1:].tolist():
            ...
            if not pd.isna(number) and 1 <= int(number) <= 12:
                return int(number)
