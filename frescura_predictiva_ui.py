...
def _style() -> None:
    st.markdown(
        ...
        /* Navegación principal: botones reales, siempre legibles */
        ...
        div[data-testid="stButton"] > button[data-testid="stBaseButton-primary"] {
            background:#28549a !important;
            color:#ffffff !important;
            border-color:#28549a !important;
            box-shadow:0 3px 8px rgba(40,84,154,.20) !important;
        }
        .frescura-title-row {
            display:flex;
            align-items:center;
...
def _render_scope_workspace(
    ...
    # Ruido operativo: los remanentes menores a 1 bulto siguen participando
    # del cálculo FEFO, pero no se muestran ni cuentan en los KPI/listados.
    forecast_scope = forecast_scope.copy()
    stock_visible = pd.to_numeric(
        forecast_scope.get("stock_lote", 0), errors="coerce"
    ).fillna(0.0)
    forecast_scope = forecast_scope.loc[stock_visible >= 1.0].copy()
    if forecast_scope.empty:
        st.info("No hay lotes con stock de al menos 1 bulto en esta vista.")
        return
...
def render_predictive_section(
    ...
    if icon_path.exists():
        ...
        icon_html = f'<img src="data:image/png;base64,{icon_b64}" alt="Mentalista">'
    else:
        icon_html = '<div style="font-size:48px;line-height:1">🔮</div>'
    st.markdown(
        f'<div class="frescura-title-row">{icon_html}<h2>Frescura predictiva</h2></div>',
        unsafe_allow_html=True,
    )
    ...
    if "frescura_pred_view" not in st.session_state:
        st.session_state["frescura_pred_view"] = "TOTAL DDV"

    nav1, nav2, nav3 = st.columns(3, gap="small")
    current_view = st.session_state["frescura_pred_view"]
    with nav1:
        if st.button(
            "TRELEW",
            key="frescura_nav_trelew",
            type="primary" if current_view == "TRELEW" else "secondary",
            use_container_width=True,
        ):
            st.session_state["frescura_pred_view"] = "TRELEW"
            st.rerun()
    with nav2:
        if st.button(
            "MADRYN",
            key="frescura_nav_madryn",
            type="primary" if current_view == "MADRYN" else "secondary",
            use_container_width=True,
        ):
            st.session_state["frescura_pred_view"] = "MADRYN"
            st.rerun()
    with nav3:
        if st.button(
            "TOTAL DDV",
            key="frescura_nav_ddv",
            type="primary" if current_view == "TOTAL DDV" else "secondary",
            use_container_width=True,
        ):
            st.session_state["frescura_pred_view"] = "TOTAL DDV"
            st.rerun()
