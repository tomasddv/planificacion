...
function doPost(e) {
  try {
    ...
    const result = payload.tipo === "kpi_promotores"
      ? guardarKpiPromotoresDesdeDash(payload)
      : guardarPlanificacionDesdeDash(payload);
...
function guardarKpiPromotoresDesdeDash(payload) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  rows.forEach((row) => {
    const data = {
      ...
      kpi: row.kpi || "",
      tarjeta: row.tarjeta || "",
      promotor: normalizarPromotor(row.promotor),
      clientesRuta: row.clientes_ruta === "" ? "" : Number(row.clientes_ruta || 0),
      restantes: row.restantes === "" ? "" : Number(row.restantes || 0),
      planificado: row.planificado === "" ? "" : Number(row.planificado || 0),
      real: row.real === "" ? "" : Number(row.real || 0),
      cumplimiento: row.cumplimiento === "" ? "" : Number(row.cumplimiento || 0),
      actualizado: updatedAt,
    };
    if (!data.fecha || !data.ruta || !data.kpi || !data.promotor) {
      return;
    }
    upsertBDKpiPromotores(ss, data);
    escritos += 1;
  });

  return { escritos, hoja: "BD_KPI_PROMOTORES" };
}

function upsertBDKpiPromotores(ss, row) {
  let sheet = ss.getSheetByName("BD_KPI_PROMOTORES");
  if (!sheet) {
    sheet = ss.insertSheet("BD_KPI_PROMOTORES");
    ...
    sheet.getRange(1, 1, 1, 11).setFontWeight("bold").setBackground("#d9eaf7");
  }
  const values = sheet.getDataRange().getValues();
  const key = `${normalizarFecha(row.fecha)}|${row.ruta}|${row.kpi}|${row.promotor}`;
  const newRow = [
    row.fecha,
    row.ruta,
    row.kpi,
  ...
  for (let r = 1; r < values.length; r++) {
    const existingKey = `${normalizarFecha(values[r][0])}|${values[r][1]}|${values[r][2]}|${normalizarPromotor(values[r][4])}`;
    if (existingKey === key) {
      sheet.getRange(r + 1, 1, 1, 11).setValues([newRow]);
      return;
    }
  }
